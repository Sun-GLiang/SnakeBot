...
export class Snake extends GameObject {
    constructor(info, gamemap) {
        ...
        this.dr = [-1, 0, 1, 0]; // 行方向的四个偏移量
        this.dc = [0, 1, 0, -1]; // 列方向的四个偏移量
    ...
    update_move() {
        ...
        if (distance < this.eps) { // 走到目标点了
            this.cells[0] = this.next_cell; // 增加一个蛇头
            this.next_cell = null;
            this.status = "idle"; // 走完了，就停下来静止
    ...
    render() {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            ctx.beginPath();
            ctx.arc(eye_x, eye_y, L * 0.05, 0, 2 * Math.PI * 2);
            ctx.fill();
        }
    }
}
